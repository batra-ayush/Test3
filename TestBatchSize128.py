...
def get_dataset(batch_size, use_cache=False):
    data_dir = '/data/imagenet_raw_data/tf_records/*'
...
print('  images/sec: %d' % results.loc['images_per_sec_mean'])
print('  latency_mean(ms): %.2f' % results.loc['latency_mean'])

file_obj = open('Outfile.txt','a')
file_obj.write('\n Inference with batch size %d\n' % batch_size)
file_obj.write('  images/sec: %.2f\n' % results.loc['images_per_sec_mean'])
file_obj.write('  latency_median(ms): %.2f\n' % results.loc['latency_median'])
file_obj.write('  latency_mean(ms): %.2f\n' % results.loc['latency_mean'])
file_obj.close()
